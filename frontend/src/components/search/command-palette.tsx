...
export function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const items: PaletteItem[] = isSearchMode ? [...clientItems, ...caseItems] : navItems;
      ...
      >
        ...
        <div className="flex items-center gap-4 border-t border-border px-4 py-2 text-xs text-muted-foreground">
          <span className="flex items-center gap-1">
            <kbd className="rounded border border-border px-1">↑↓</kbd> навигация
          </span>
          <span className="flex items-center gap-1">
            <kbd className="rounded border border-border px-1">Enter</kbd> перейти
          </span>
        </div>
      </div>
    </div>
